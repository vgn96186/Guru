...
- [ ] Add bounds checking on `agenda.items[currentItemIndex]` access everywhere
- [ ] Move `elapsedSeconds` and `activeElapsedSeconds` into the Zustand store so they survive crashes
- [ ] Extract confidence-to-status mapping into a pure function in `src/services/xpService.ts`
- [ ] Remove the unused `quiz` session state
- [ ] Add auto-retry with 2-attempt backoff to the content loading effect before showing error UI

### Phase 2: Component Decomposition (medium risk, high impact)

- [ ] Extract `WarmUpMomentumScreen` to `src/screens/WarmUpMomentumScreen.tsx`
- [ ] Extract `SessionDoneScreen` to `src/screens/SessionDoneScreen.tsx`
- [ ] Extract `SessionStudyingView` for the main studying render path (lines 739-931)
- [ ] Extract `SessionHeader` component (topic name, pause, menu)
- [ ] Extract `SessionMenu` overlay component

### Phase 3: State Machine Formalization (medium risk, high impact)

- [ ] Create a `sessionStateMachine.ts` with explicit valid transitions:
...
- [ ] Replace direct `setSessionState` calls with a `transitionTo` function that validates the transition
- [ ] Consolidate `finishSession` to only be called from the state machine transition, removing the effect + manual call pattern

### Phase 4: Session Planner Cleanup (low risk, medium impact)

- [ ] Refactor `buildSession` to accept a single config object instead of 6 positional args
- [ ] Move API key resolution inside the planner (it already reads profile internally)
- [ ] Fix the `downgradeSession` type cast by making the Agenda type accept optional `mode` override

### Phase 5: ADHD UX Polish (low risk, high UX impact)

- [ ] Add haptic + notification when break auto-ends
- [ ] Add content loading skeleton instead of LoadingOrb for faster perceived performance
- [ ] Show a mini session summary in the Leave Session alert (topics done, minutes, XP)
- [ ] Add progressive break reminders (vibrate at 30s, stronger at 10s, alarm at 0s)
