...
describe('DailyAgendaSection', () => {
  const mockTasks: TodayTask[] = [
    {
      topic: { id: '1', name: 'Task 1', subjectId: 's1', subjectName: 'Subject 1' } as any,
      timeLabel: '10:00 AM - 11:00 AM',
      type: 'review',
    },
    {
      topic: { id: '2', name: 'Task 2', subjectId: 's2', subjectName: 'Subject 2' } as any,
      timeLabel: '11:00 AM - 12:00 PM',
      type: 'deep_dive',
    },
    {
      topic: { id: '3', name: 'Task 3', subjectId: 's3', subjectName: 'Subject 3' } as any,
  ...
  it('renders empty state when no tasks and hasNewTopics is true', () => {
    const { getByText } = render(
      <DailyAgendaSection {...defaultProps} hasNewTopics={true} />
    );
  ...
  it('renders list of tasks when todayTasks is not empty', () => {
    const { getByText } = render(
      <DailyAgendaSection {...defaultProps} todayTasks={mockTasks} />
    );
    ...
    expect(getByText(/NEW · Subject 3/i)).toBeTruthy();
