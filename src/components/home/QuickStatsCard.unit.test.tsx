...
describe('QuickStatsCard', () => {
  ...
  it('renders correctly with default props', () => {
    ...
    expect(getByLabelText('Your progress today. 45% of daily goal. 5 day streak. Level 12. 3 sessions.')).toBeTruthy();
  });
  ...
  it('renders goal complete message when progressPercent is 100', () => {
    const { getByText } = render(<QuickStatsCard {...defaultProps} progressPercent={100} todayMinutes={100} />);
  ...
  it('renders goal complete message when progressPercent exceeds 100', () => {
    const { getByText, getByLabelText } = render(<QuickStatsCard {...defaultProps} progressPercent={120} todayMinutes={120} />);
    expect(getByText('Daily goal complete. Stack one more high-yield block.')).toBeTruthy();
    // Clamped to 100% in display and label
    expect(getByText('100%')).toBeTruthy();
    expect(getByLabelText('Your progress today. 100% of daily goal. 5 day streak. Level 12. 3 sessions.')).toBeTruthy();
  });
  ...
  it('handles negative todayMinutes gracefully in remaining time calculation', () => {
    ...
    // dailyGoal(100) - todayMinutes(-10) = 110
    expect(getByText('110 min left to hit today target.')).toBeTruthy();
  });

  it('handles negative progressPercent by clamping to 0%', () => {
    const { getByText, getByLabelText } = render(<QuickStatsCard {...defaultProps} progressPercent={-10} todayMinutes={-10} />);
    expect(getByText('0%')).toBeTruthy();
    expect(getByLabelText('Your progress today. 0% of daily goal. 5 day streak. Level 12. 3 sessions.')).toBeTruthy();
  });
});
