...
const LANDSCAPE_BREAKPOINT = 900;
...
export function useResponsive() {
  ...
  const isLandscape = width >= LANDSCAPE_BREAKPOINT && width > height;
  // Portrait tablets: cap at 800. Landscape: use 95% of screen width so it fills the device.
  const maxContentWidth = isTablet
    ? (isLandscape ? Math.round(width * 0.95) : Math.min(800, Math.round(width * 0.95)))
    : undefined;
