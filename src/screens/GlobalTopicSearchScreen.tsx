          ...
          <FlatList
            ...
            ListEmptyComponent={
              query.length >= 2 ? (
                ...
                <View style={styles.emptyState}>
                  <Ionicons name="search-outline" size={48} color={theme.colors.border} style={{marginBottom: 16}} />
                  <Text style={styles.emptySub}>Type at least 2 characters to search across all subjects.</Text>
                </View>
              )
            }
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
                onPress={() => navigation.navigate('Session', { mood: 'good', focusTopicId: item.id })}
              >
...
const styles = StyleSheet.create({
  ...
  searchContainer: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: theme.colors.surfaceAlt, borderRadius: theme.borderRadius.md, paddingHorizontal: 12, height: 44, borderWidth: 1, borderColor: theme.colors.border },
  searchIcon: { marginRight: 8 },
  searchInput: { flex: 1, color: theme.colors.textPrimary, fontSize: 16, height: '100%' },
  clearBtn: { padding: 4 },
  listContent: { padding: 16 },
  resultItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: theme.colors.surface, padding: 16, borderRadius: theme.borderRadius.md, marginBottom: 8, borderWidth: 1, borderColor: theme.colors.border },
