...
export default function NotesVaultScreen() {
  ...
  const {
    ...
    initialSortBy: 'date',
    pageSize: PAGE_SIZE,
    filterItem: (note: NoteItem, search: string, subj: string, top: string) => {
      if (subj !== 'all' && (note.subjectName || 'Unknown') !== subj) return false;
      if (top !== 'all' && !note.topics.includes(top)) return false;
    ...
    sortItems: (a: NoteItem, b: NoteItem, sort: string) => {
  ...
  const subjectOptions = useMemo((): string[] => {
    const subjects = notes.map((note) => note.subjectName || 'Unknown');
    ...
    ({ item }: { item: NoteItem }) => {
      return (
        <NoteCardItem
          ...
          onPress={(note: NoteItem) => {
            if (isSelectionMode) {
              void Haptics.selectionAsync();
              toggleSelection(note.id);
              return;
            }
            setReaderTitle(getTitle(note));
            setReaderContent(note.note);
            setReaderNote(note);
          }}
