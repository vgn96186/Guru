/**
 * OpenAI Responses API adapter for LanguageModelV2.
 *
...
export function createResponsesApiModel(config: ResponsesApiConfig): LanguageModelV2 {
  const doFetch = config.fetch ?? fetch;

  const buildBody = (options: LanguageModelV2CallOptions, stream: boolean) => {
      ...
      for (const line of rawEvent.split('\n')) {
        ...
        switch (evt.type) {
          ...
          case 'response.reasoning_summary_text.delta': {
            const delta: string = evt.delta ?? '';
            if (!delta) break;
            // Route via id prefix so streamText maps to reasoning-delta.
            if (!textStarted) {
              textStarted = true;
              yield { type: 'text-start', id: textId };
            }
            yield { type: 'text-delta', id: `reasoning-${textId}`, delta };
            break;
          }
