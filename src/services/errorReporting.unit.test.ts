...
describe('errorReporting', () => {
  ...
  it('reports error to local DB and Sentry', () => {
    jest.isolateModules(() => {
      ...
      expect(logErrorToDatabase).toHaveBeenCalledWith(expect.objectContaining({
        error: 'Test error',
        context: JSON.stringify({ contextKey: 'contextValue' }),
      }));
      
      expect(SentryMock.captureException).toHaveBeenCalledWith(error, { extra: { contextKey: 'contextValue' } });
    });
  });
  ...
  it('reports string error to local DB and Sentry', () => {
    jest.isolateModules(() => {
      ...
      expect(SentryMock.captureMessage).toHaveBeenCalledWith('String error', { extra: { contextKey: 'contextValue' } });
    });
  });
