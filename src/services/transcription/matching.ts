...
export async function markTopicsFromLecture(
  ...
  // 1-3: Keyword matching (Batched to prevent N+1 queries)
  ...
  if (sanitizedTopics.length > 0) {
    const batchedMatches = await findTopicIdsByKeywordsBatched(db, sanitizedTopics, subjectName);
    for (const { matchId } of batchedMatches) {
