...
let query = db
    .select({
      ...
      childCount: sql`(SELECT COUNT(*) FROM topics c WHERE c.parent_topic_id = topics.id)`,
    })
    .from(topics)
...
// Also try to execute the query without the aliases directly via better-sqlite3
try {
  const result = sqlite.prepare(query.toSQL().sql).all(query.toSQL().params);
  console.log("FIELDS returned by sqlite:", Object.keys(result[0] || {}));
} catch(e) {}
